data {
  ...
  int<lower=0> SpSp;
  int<lower=1, upper=P> first_player[M];            // ID number of first player
  int<lower=1, upper=P> second_player[M];           // ID number of second player
  int<lower=1, upper=St> first_starter[M];           // ID number of first starter deck
  int<lower=1, upper=St> second_starter[M];          // ID number of second starter deck
  int<lower=1, upper=Sp> first_specs1[M];
  int<lower=1, upper=Sp> first_specs2[M];
  int<lower=1, upper=Sp> first_specs3[M];
  int<lower=1, upper=Sp> second_specs1[M];
  int<lower=1, upper=Sp> second_specs2[M];
  int<lower=1, upper=Sp> second_specs3[M];
  int<lower=1, upper=St*Sp> first_starter_specs1[M];
  int<lower=1, upper=St*Sp> first_starter_specs2[M];
  int<lower=1, upper=St*Sp> first_starter_specs3[M];
  int<lower=1, upper=St*Sp> second_starter_specs1[M];
  int<lower=1, upper=St*Sp> second_starter_specs2[M];
  int<lower=1, upper=St*Sp> second_starter_specs3[M];
  int<lower=1, upper=SpSp> first_spec_specs1[M];
  int<lower=1, upper=SpSp> first_spec_specs2[M];
  int<lower=1, upper=SpSp> first_spec_specs3[M];
  int<lower=1, upper=SpSp> second_spec_specs1[M];
  int<lower=1, upper=SpSp> second_spec_specs2[M];
  int<lower=1, upper=SpSp> second_spec_specs3[M];
...
transformed parameters{
  ...
  vector[M] first_spec_spec1;
  vector[M] first_spec_spec2;
  vector[M] first_spec_spec3;
  vector[M] second_spec_spec1;
  vector[M] second_spec_spec2;
  vector[M] second_spec_spec3;
  ...
  real<lower=0> sd_spec_spec = exp(lsd_spec_spec);
  ...
  vector[SpSp] spec_spec = sd_spec_spec * spec_spec_std;
  for (i in 1:M) {
  ...
  first_spec_spec1 = spec_spec[first_spec_specs1];
  first_spec_spec2 = spec_spec[first_spec_specs2];
  first_spec_spec3 = spec_spec[first_spec_specs3];
  second_spec_spec1 = spec_spec[second_spec_specs1];
  second_spec_spec2 = spec_spec[second_spec_specs2];
  second_spec_spec3 = spec_spec[second_spec_specs3];
  matchup = turn +
    ...
    first_spec_spec1 + first_spec_spec2 + first_spec_spec3 -
    second_spec_spec1 - second_spec_spec2 - second_spec_spec3;
}
model {
