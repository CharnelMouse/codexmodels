...
parameters {
  vector<upper=0>[P] player_std;                    // player skill levels in log odds effect
...
model {
  ...
  // player_std is half-normal, since bounded above by 0. Actaully mean/variance are sig sqrt(2/pi) and sig^2(1 - 2/pi).
  // standard deviation is picked to make actual variance 1, to keep it as a standard measure.
  // mean is about -1.32.
  player_std ~ normal(0, 1/sqrt(1 - 2/pi()));
  for (i in 1:St) {
